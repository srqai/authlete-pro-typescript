...
import { AutheleteProDefaultError } from "../models/errors/autheleteprodefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AutheleteProDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AutheleteProDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AutheleteProDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AutheleteProDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
